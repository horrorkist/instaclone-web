...
import { UPLOAD_POST_MUTATION, UPLOAD_URL_QUERY } from "../../libs/queries";
...
function CreatePostModal({ exit }: { exit: () => void }) {
  ...
  return (
    <div className="w-[1200px] h-[900px] rounded-xl overflow-hidden bg-white flex flex-col relative animate-fade-in">
      {(status === "loading" ||
        status === "completed" ||
        status === "failed") && (
